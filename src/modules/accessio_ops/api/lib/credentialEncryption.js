// ============================================================================
// Accessio Operations Module — Secure Token Encryption Utility
//
...
//
// ENV: ACCESSIO_TOKEN_SECRET — 32+ char secret for key derivation.
//      Falls back to JWT_SECRET, then to a default (with warning).
//
// PATTERN SOURCE: Identical to Google GKE module's credentialEncryption.js
// ============================================================================
import crypto from 'crypto';
import { createAoLogger } from './moduleLogger.js';

const log = createAoLogger('credentialEncryption');
...
const SALT = 'pulseops-accessio-token-salt-v1'; // Static salt — key uniqueness comes from the secret
...
function getEncryptionKey() {
  ...
  const secret = process.env.ACCESSIO_TOKEN_SECRET
    || process.env.JWT_SECRET
    || 'pulseops-default-accessio-token-key-change-in-production';

  if (!process.env.ACCESSIO_TOKEN_SECRET && !process.env.JWT_SECRET) {
    log.warn('ACCESSIO_TOKEN_SECRET not set — using default key. Set this env var in production!');
  }
...
export function isEncrypted(value) {
  ...
  // Check if it's our encrypted format (base64 with sufficient length and doesn't look like JWT)
  // JWTs start with "eyJ" (base64 of '{"'). Encrypted tokens don't.
  // Also check for minimum length - encrypted tokens are longer due to IV + authTag overhead
  const isJwtFormat = value.startsWith('eyJ') || value.startsWith('Bearer ');
  const hasMinLength = value.length > 100; // Encrypted tokens are typically longer
  
  return !isJwtFormat && hasMinLength;
}
